...
@Component({
  ...
  template: `
    <div class="grade-container">
      ...
      @if (horarioService.errorMessage()) { <div class="alert alert-error"><span>{{ horarioService.errorMessage() }}</span><button type="button" class="btn-retry" (click)="recarregar()">Tentar novamente</button></div> }
        ...
        <div class="timetable-grid">
          @for (dia of diasSemana; track dia) {
            <div class="dia-coluna">
              <div class="dia-header"><h3>{{ labelsDias[dia] }}</h3></div>
              <div class="dia-corpo">
                @if (obterAulasDoDia(dia).length === 0) { <div class="sem-aula">Sem aulas</div> }
                @else {
                  @for (aula of obterAulasDoDia(dia); track aula.id) {
                    <div class="aula-card">
                      <div class="aula-horario tabular-nums">{{ formatarHorario(aula.hora_inicio) }} - {{ formatarHorario(aula.hora_fim) }}</div>
                      ...
                      <div class="aula-detalhe">
                        @if (perfil() === 'aluno' && aula.professor_nome) { <span class="prof-label">Docente: {{ aula.professor_nome }}</span> }
  ...
  styles: [`
    ...
    .timetable-grid { display: grid; grid-template-columns: repeat(6, 1fr); gap: 0.75rem; align-items: start; }
    @media (max-width: 1024px) { .timetable-grid { grid-template-columns: repeat(3, 1fr); } }
